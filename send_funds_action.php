    ...
    if (($result0->num_rows) > 0) {
        ...
        if($updated_sender_balance >= 0) {
            ...
            $sql3 = "INSERT INTO passbook".$sender_id." VALUES(
                        NULL,
                        NOW(),
                        'Trimis către: ".$row5["first_name"]." ".$row5["last_name"].", IBAN: ".$row5["account_no"]."',
                        '$amt',
                        '0',
            ...
            $sql4 = "INSERT INTO passbook".$receiver_id." VALUES(
                        NULL,
                        NOW(),
                        'Primit de la: ".$row0["first_name"]." ".$row0["last_name"].", IBAN: ".$row0["account_no"]."',
                        '0',
                        '$amt',
...
<body>
    <div class="flex-container">
        <div class="flex-item">
            ...
            <?php
            if ($err_no == 0) { ?>
                <p id="info"><?php echo "Tranzactie cu succes !\n"; ?></p>
            <?php } ?>

            <?php
            if ($err_no == 1) { ?>
                <p id="info"><?php echo "Fonduri nsuficiente !\n"; ?></p>
            <?php } ?>

            <?php
            if ($err_no == 2) { ?>
                <p id="info"><?php echo "Parolă greșită !\n"; ?></p>
            <?php } ?>
        </div>
